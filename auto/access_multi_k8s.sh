...
# cluster join
ssh ${FIRST_CLUSTER_MASTER} "sudo -i cat ~root/join_token" | echo > k8s-master-join
ssh ${FIRST_CLUSTER_WORKER1} "sudo -i $( cat ./k8s-master-join)"
ssh ${SECOND_CLUSTER_MASTER} "sudo -i cat ~root/join_token" | echo > hk8s-master-join
ssh ${FIRST_CLUSTER_WORKER1} "sudo -i $( cat ./hk8s-master-join)"
